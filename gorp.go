...
func (c *ColumnMap) TypeDef() string {
	def, _ := c.typeDef(true)
	return def
}

// TypeDefNoNotNull returns the type definition for this column, minus
// not null constraints.  This is mainly for migration, so that when a
// new column is added, data can be updated prior to adding the not
// null constraint.
//
// If a NOT NULL constraint was ignored, the returned bool will be
// true.
func (c *ColumnMap) TypeDefNoNotNull() (string, bool) {
	return c.typeDef(false)
}
